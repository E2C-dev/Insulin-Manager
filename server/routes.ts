...
export async function registerRoutes(
  ...
  app.post("/api/insulin-entries", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // つぶされる設計)。
      // autoCalculated / liveMeasurementSlot は insertInsulinEntrySchema
      // (DBカラムに対応するスキーマ) には含まれず parse() で strip されるため、
      // req.body から直接読む。
      await checkInsulinDoseAndLog(req, entry, {
        autoCalculated: req.body?.autoCalculated === true,
        liveMeasurementSlot: typeof req.body?.liveMeasurementSlot === "string" ? req.body.liveMeasurementSlot : undefined,
      });
